...
**Theorem.** There is a universal constant $C$ such that ForceMoM, for every $\rho\in(0,1)$, $p\in(1,2]$, $u>0$, and $T\geq1$, satisfies, with $\gamma_p=(p-1)/p$, 
...
$$
 \limsup_{T\to\infty}\frac{R_T(\nu)}{T^\rho} \leq K^{-\rho}\sum_{i=1}^K\Delta_i. 
$$

**Theorem.** Fix $p\in(1,2]$. Suppose a policy, without knowing $u$, has a moment-free distribution-free bound $\Phi_p(T)=o(T)$ as in (eq. in paper). Then on every fixed instance $\nu$ with at least one suboptimal arm, 

$$
 \liminf_{T\to\infty} \frac{R_T(\nu)}{(T/\Phi_p(T))^{p/(p-1)}} \geq c_p\sum_{i:\Delta_i>0}\Delta_i, 
$$

 where $c_p>0$ depends only on $p$. Hence the achievable polynomial exponents obey a matching lower tradeoff, stated in full in the paper.

**Corollary.** Fix any $\rho\in(0,2/3]$. One policy, independent of $p$ and $u$, attains for every $p\in(1,2]$ the polynomial exponents 

$$
 (a_p,b)=\left(1-\rho\frac{p-1}{p},\rho\right). 
$$

 These exponents satisfy the lower tradeoff with equality for every $p$. The slowly varying gap in (eq. in paper) can be made arbitrarily small by the choice of $\ell$.
